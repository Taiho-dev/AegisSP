	...
	reload() {
		...
		if(this.mod.settings.debug.skills) {
			...
			for(let options of [
				...
				['C_START_COMBO_INSTANT_SKILL', 6, _ => `[${_.targets.map(t => `${t.unk1} ${gameId(t.targets)} ${t.unk2}`).join(',')}]`],
				['C_START_INSTANCE_SKILL', 7, _ => [_.unk, `[${_.targets.map(t => `${t.unk1} ${gameId(t.target)} ${t.unk2}`).join(',')}]`]],
