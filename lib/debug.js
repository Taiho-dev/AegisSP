...
class SPDebug {
	...
	reload() {
		...
		if(this.mod.settings.debug.skills) {
			...
			for(let options of [
				['C_START_SKILL', 7, _ => [Number(_.unk), Number(_.moving), Number(_.continue), `Target: ${Number(_.target)}`]],
				['C_PRESS_SKILL', 4, _ => _.press],
				['C_START_TARGETED_SKILL', 7, _ => `[${_.targets.map(t => `gameId: ${Number(t.gameId)}' 'hitCylinder: ${Number(t.hitCylinderId)}`).join(',')}]`],
				['C_START_COMBO_INSTANT_SKILL', 6, _ =>{ 
				 if (!_.targets || !_.targets.length) return '[]';
  				return `[${_.targets.map(t => ` 'arrowId: ${t.arrowId}' 'gameid: ${t.gameId}' 'hitCylinderid: ${t.hitCylinderId}`).join(',')}]`;
				}],
				['C_START_INSTANCE_SKILL', 7, _ => [_.unk, `[${_.targets.map(t => `${t.arrowId} ${t.gameId ? t.gameId.toString() : '0'} ${t.hitCylinderId}`).join(',')}]`]],
				['C_START_INSTANCE_SKILL_EX', 5, _ => [_.unk, (_.projectile)]],
