...
(Server bug: Having Noctenium Infusion in inventory causes skill delays)`)
		...
		if(event.continue)
			...
			if(!tryCancelSkill(endReason, info, type, event, data)) {
				sendCannotStartSkill(event.skill)
				return false
			}
		...
		//intercept for chains like balders
		if (!event.skill.equals(skill)) 
			{
			event.skill = skill;
			let version = mod.dispatch.protocolMap.name.get(type); 
			if (version) 
				{
				data = mod.dispatch.toRaw(type, event._version, event);
				}
		}
	...
	function tryCancelSkill(endReason, nextSkill) 
	{
		...
		const { allow_cancel, pending_remaining_time} = canCancelSkill(endReason, nextSkill)
				...
				end_timer = mod.setTimeout(() => 
			{
				...
				if (handleStartSkill(type, event, nextSkill, data) !== false) 
					{
					toServerLocked(data);
					}
	...
	function startAction(opts) {
		...
		// work around to fix skill speed when abnormal applies extra speed on current skill. To align packets properly with the server.
		// this will be needed later to emulate glyphs speed bonus that rely on abnormalities
		if(info.TriggerPreCalculateSpeedAbnormals)
			for (let id in info.TriggerPreCalculateSpeedAbnormals)
			{
			const PreAbnormal = info.TriggerPreCalculateSpeedAbnormals[id];
			if(Array.isArray(info.PreCalculateSpeedAbnormals))
			{	
