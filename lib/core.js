...
module.exports = function SPCore(mod) {
	...
	//code ripped from release chain, it does look ugly but it works
		...
		if (info.connectChains[current_skill_id]) 
			{
				const next_skill_id = info.connectChains[current_skill_id];
				const prevInfo = skillInfo(clientAction.skill);
			...
			if (prevInfo) 
				...
				if(
					elapsed >= required_time || event.ready_to_connect) 
					{
...
(Server bug: Having Noctenium Infusion in inventory causes skill delays)`)
	...
	function canCancelSkill(endReason, nextSkill) 
	{
		...
		//what dc types? //just a guess for now
