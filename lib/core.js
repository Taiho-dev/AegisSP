...
(Server bug: Having Noctenium Infusion in inventory causes skill delays)`)
	...
	function tryCancelSkill(endReason, nextSkill) 
	{
		...
		const { allow_cancel, cancel_remaining_time} = canCancelSkill(endReason, nextSkill)
			...
			{
           		end_timer = null;
          	  	tryCancelSkill(endReason, nextSkill);
	...
	function canCancelSkill(endReason, nextSkill) 
	{
		...
		//which ones are considered for pendingstarttime
		...
		const valid_types_for_rear_cancel = ['rearCancel','rearCancelOnly']		
		const rear_cancel_valid_types = valid_types_for_rear_cancel.includes(next_skill_pending_type);

		if (current_info && current_info.cancelLogic && current_info.pendingStartTime && pending_valid_types){
		...
		if (devmode){
		console.log(`DevMode: Cancel Denied for pendingTypes(elapsed: ${Math.round(pending_elapsed)}ms | required: ${Math.round(pending_required)}ms)`);
		}
		return { allow_cancel: false, cancel_remaining_time: pending_remaining_time };
		}
		
		if (devmode){
		console.log(`DevMode: Cancel Granted for pendingTypes(elapsed: ${Math.round(pending_elapsed)}ms | required: ${Math.round(pending_required)}ms)`);
		}
		}
