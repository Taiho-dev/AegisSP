...
module.exports = function SPCore(mod) {
	...
	// when skills end too early due to s_action_end, we have to keep the remaining pendingstarttime in memory for the next skill
	// TO DO
		end_timer = null,	
		linger_pending = 0,
		next_linger_id = [],
		pending_end_time = 0,
		pending_lingering = 0,
	//For Release Chains such as mana missile.	
	//Adds release multiplied by ASPD to ReleaseChains, modify to decrease desyncronization.
		release_delay = 0,
	// add delay to when you are allowed to cancel a skill when cancelLogic is being used. increase when you still desync.
		cancel_delay = 5,
	// Simulate/Adds Server tickrate. Value is in hertz.
	// Rolling reload - Mana missile desyncs without considering server tick rate. When the delay is too exact.
	// Math is based on Best Case Scenario
		tick_rate = 30,
	// Projectiles
...
(Server bug: Having Noctenium Infusion in inventory causes skill delays)`)
	...
	function tryCancelSkill(endReason, nextSkill) 
	{
		...
		const { allow_cancel, pending_remaining_time} = canCancelSkill(endReason, nextSkill)
			if (!allow_cancel)
			{		
			...
			pending_end_time = Date.now() + pending_remaining_time;
		...
		if (end_timer)
			{
			 ...
			 end_timer = null;
			 pending_lingering = Math.max(0, pending_end_time - Date.now());
			}
			else{
				pending_lingering = 0;
	...
	function canCancelSkill(endReason, nextSkill) 
	{
		...
		if (current_info && current_info.cancelLogic && current_info.pendingStartTime){
		...
		//we gonna store remaining time to queue up the next attempt
		const pending_remaining_time = Math.max(0,pending_required - pending_elapsed );

		if (devmode){
		console.log(`DevMode: Cancel Denied (elapsed: ${Math.round(pending_elapsed)}ms | required: ${Math.round(pending_required)}ms)`);
		}
		return { allow_cancel: false, pending_remaining_time };
		}
		if (devmode){
		console.log(`DevMode: Cancel Granted (elapsed: ${Math.round(pending_elapsed)}ms | required: ${Math.round(pending_required)}ms)`);
		}
		}	
		}
		return { allow_cancel: true, pending_remaining_time: 0 };
	}
