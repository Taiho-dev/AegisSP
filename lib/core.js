...
(Server bug: Having Noctenium Infusion in inventory causes skill delays)`)
	...
	function canCancelSkill(endReason, nextSkill) 
	{
		...
		const cancel_extra_delay = cancel_delay / clientAction.speed;
		const pending_required = (current_info.pendingStartTime / clientAction.speed) + final_tick_rate + cancel_extra_delay;
		...
		const final_tick_rate = (1/2) * (1000 / tick_rate); 	
		const cancel_extra_delay = cancel_delay / clientAction.speed;
		const rear_cancel_required = (current_info.rearCancelStartTime / clientAction.speed) + final_tick_rate + cancel_extra_delay;
