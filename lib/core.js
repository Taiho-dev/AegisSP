...
(Server bug: Having Noctenium Infusion in inventory causes skill delays)`)
		...
		if (end_timer)
			{
			 ...
			 end_timer = null;
			 cancel_lingering = Math.max(0, pending_end_time - Date.now());
			}
			else{
				cancel_lingering = 0;
	...
	function canCancelSkill(endReason, nextSkill) 
	{
		...
		const pending_required = (current_info.pendingStartTime / clientAction.real_speed) + final_tick_rate + cancel_extra_delay;
