...
module.exports = function SPCore(mod) {
	...
	// adds additional delay to releasechains.
		release_delay = 0,
	// adds additional delay to cancel logic.
		cancel_delay = 5,
	...
	for (let releaseChain of releaseChains)
	{
		...
		{
			// Abnormality check with polishing with ararys fore releasechains.
		if	(	
...
(Server bug: Having Noctenium Infusion in inventory causes skill delays)`)
	...
	function tryCancelSkill(endReason, nextSkill, type, event, data) 
	{
		const { allow_cancel, pending_remaining_time} = canCancelSkill(endReason, nextSkill)
			if (!allow_cancel)
			{		
			// we have to store the remaining pending start time because s_action_end will clear it prematurely it.
				...
				end_timer = mod.setTimeout(() => 
			{
				...
				if (handleStartSkill(endReason, nextSkill, type, event, data) !== false) 
					{
					toServerLocked(data);
					}
	...
	function startAction(opts) {
		...
		//method to pre-apply speed buffs before skill is being cast. simulates attack speed emulation from abnormals.
	...
	function sendActionStage(opts) {
		...
		if (!info.ignoreAttackSpeed && preCalculatedSpeeds.size > 0) 
			...
			else if(preCalculatedSpeeds.size > 0){
			{	
