...
module.exports = function SPCore(mod) {
	...
	//storing value for temporary speed 
		preCalculatedSpeeds = new Map(),
	...
	// Current skill
		...
		// for the cancel logic
		end_timer = null,	
		cancel_end_time = 0,
		cancel_lingering = 0,
		// increase to add delay to release chains
		release_delay = 0,
		// add delay to when you are allowed to cancel a skill when cancelLogic is being used. increase when you still desync.
		cancel_delay = 0,
		// adds tickrate, value is in hertz. based on best case scenario half-tick latency.
		tick_rate = 60,
		// Projectiles
	...
	// debugging purposes
	mod.command.add('devmode', () => {
    devmode = !devmode; 
    mod.command.message(`devmode: ${devmode ? 'on' : 'off'}`);
	});

	mod.command.add('reloadskills', () => {
		try {	delete require.cache[require.resolve('../config/skills')];
				skills = require('../config/skills');
				skillsCache.clear(); 
				mod.command.message(`Skills Reloaded`);
			}
		catch (error) 
			{	mod.command.message(`error, skills reload failed`)
			}	
	});
	...
	// Modified code to support arrays, which is needed for RR -> MM chain logic.			
	let releaseChains = info.releaseChain ? (Array.isArray(info.releaseChain) ? info.releaseChain : [info.releaseChain]) : []
	let doReleaseChain = false;

	//loop through all available release chains.

	for (let releaseChain of releaseChains)
	{
			// Skill override (release chain)
		if	(	
			releaseChain && clientAction &&
		...
		{
			// Modification to require/exclude abnormals so release chain knows which one to use based on active abnormalities.
		if	(	
			(releaseChain.RequireAbnormal && !releaseChain.RequireAbnormal.every(id => abnormality.exists(id))) ||
			(releaseChain.ExcludeAbnormal && releaseChain.ExcludeAbnormal.some(id => abnormality.exists(id)))	||
			(releaseChain. requirePolishing &&!releaseChain.requirePolishing.every(id =>player.skillPolishing.has(id)))	
			)
			{
			continue;
			}
            ...
            if (prevInfo) {
                const required_time = prevInfo.pendingStartTime / clientAction.speed;
                const elapsed = Date.now() - lastStartTime;
				
                if (elapsed >= required_time || event.ready_to_release) {                
				...
				//Temporary, will place it somewhere else in the future
					const best_case_scenario = (1/2) * (1000 / tick_rate);	
					const final_tick_rate = best_case_scenario;

					const additional_time = release_delay / clientAction.speed;					
                    const remaining_time = required_time - elapsed + final_tick_rate + additional_time;
                    
                    mod.clearTimeout(delayedSkillTimeout);
                    delayedSkillTimeout = mod.setTimeout(() => {
					//Fire it if true!
                    event.ready_to_release = true;
                        
                    
                    if (handleStartSkill(type, event, info, data) !== false) {
                        toServerLocked(data);
                        }
                    }, remaining_time);
	...
	//code ripped from release chain, atleast it works!
		let doConnectChain = false;

		if (info.connectChains && clientAction){
			const current_skill_id = clientAction.skill.id;
			const chain_info = info.connectChains[current_skill_id];

			if (chain_info){
				const next_skill_id = chain_info.connectToID;
				const rear_this = chain_info.type === 'rear';
				const prevInfo = skillInfo(clientAction.skill);

				if (prevInfo){
					const number = rear_this ? prevInfo.rearCancelStartTime : prevInfo.pendingStartTime;
					const required_time = number / clientAction.speed; 
					const elapsed = Date.now() - lastStartTime;

					if (elapsed >= required_time || event.ready_to_connect){
						skill = modifyChain(skill, next_skill_id);
						...
						const best_case_scenario = (1 / 2) * (1000 / tick_rate);
						const final_tick_rate = best_case_scenario;
						const additional_time = cancel_delay / clientAction.speed;
						const remaining_time = required_time - elapsed + final_tick_rate + additional_time;

						mod.clearTimeout(delayedSkillTimeout);
						delayedSkillTimeout = mod.setTimeout(() => {
							event.ready_to_connect = true;
							if (handleStartSkill(type, event, info, data) !== false) {
								toServerLocked(data);
							}
						}, remaining_time);
		...
		if(clientAction) {
			...
			if(clientAction.skill.type === TYPE_ACTION) {
				...
				if (chain == null && info.abnormalChains) {
...
(Server bug: Having Noctenium Infusion in inventory causes skill delays)`)
		...
		if(event.continue)
			...
			if(info.connectSkillArrow) {
				...
				if(info.consumeAbnormalArrow) {
				if(Array.isArray(info.consumeAbnormalArrow)) 
					{
					for(let id of info.consumeAbnormalArrow) abnormality.remove(id)
					} 
					else {
					abnormality.remove(info.consumeAbnormalArrow)
					}
				}
				updatePosition(event, false, specialLoc)
				sendActionEnd(4)
		...
		// However, once the animation starts this is no longer possible, so instead we simulate retrying each skill
			...
			if(devmode)
				{
				console.log(`DevMode: inspect packet rewire something ${event.skill.id} to ${skill.id}`);
			}
		retry('skill', info, () => toServerLocked(data))
	}

	function tryCancelSkill(endReason, nextSkill, cancel_call_back) 
	{
		const { allow_cancel, cancel_remaining_time} = canCancelSkill(endReason, nextSkill)
			if (!allow_cancel)
			{
				
				if (end_timer) 
					{
						if (devmode) console.log("DevMode: Will this work against button mashing?");
				return false;
					}

			cancel_end_time = Date.now() + cancel_remaining_time;
				if (end_timer) mod.clearTimeout(end_timer);

				end_timer = mod.clearTimeout(end_timer);

				end_timer = mod.setTimeout(() =>
			{
           		end_timer = null;
          	  	if (tryCancelSkill(endReason, nextSkill))
				{
					if (cancel_call_back) cancel_call_back();
				}

			if (devmode)
				{
				console.log(`DevMode: This is a debugger to see if it will try again`);
				}
		...
		if (end_timer)
			{
			 mod.clearTimeout(end_timer);	
			 end_timer = null;
			 cancel_lingering = Math.max(0, cancel_end_time - Date.now());
	...
	function canCancelSkill(endReason, nextSkill) 
	{
		if(clientAction)
			{
		const next_skill_pending_type = nextSkill.skillPendingType;		
		const current_info = skillInfo(clientAction.skill);
		const next_skill_dc_type = nextSkill.dcType;

		//redundant buggy.
	
		const valid_types_for_pending = ['immediateCancel','normal'];
		const valid_types_for_rear_cancel = ['rearCancel'];
		
		const valid_dc_types_for_pending =['connect','change', 'normal','combo_instance'];
		const valid_dc_types_for_rear_cancel =['Evade','Defence', 'Shorttel','stone' ];
		
		const dc_valid_types_pending_includes = valid_dc_types_for_pending.includes(next_skill_dc_type);
		const dc_valid_types_rear_cancel_includes = valid_dc_types_for_rear_cancel.includes(next_skill_dc_type);

		const pending_valid_types = valid_types_for_pending.includes(next_skill_pending_type);	
		const rear_cancel_valid_types = valid_types_for_rear_cancel.includes(next_skill_pending_type);

		if (current_info && current_info.cancelLogic && current_info.pendingStartTime && pending_valid_types && dc_valid_types_pending_includes){
		const final_tick_rate = (1/2) * (1000 / tick_rate); 	
		const cancel_extra_delay = cancel_delay / clientAction.real_speed;
		let refine = 1;

		if (current_info.refineChainLogic){
		refine = 2/5;
		}
		const pending_required = (((current_info.pendingStartTime / clientAction.real_speed) + final_tick_rate + cancel_extra_delay)* (refine));
		const pending_elapsed = Date.now() - lastStartTime;

		if (pending_elapsed < pending_required) {
		//we gonna store remaining time to queue up the next attempt for pendingtypes
		const pending_remaining_time = Math.max(0,pending_required - pending_elapsed );

		if (devmode){
		console.log(`DevMode: Cancel Denied for pendingStartTime(elapsed: ${Math.round(pending_elapsed)}ms | required: ${Math.round(pending_required)}ms | Real Pending: ${(current_info.pendingStartTime)} r.spd: ${(clientAction.real_speed)})`);
		}
		return { allow_cancel: false, cancel_remaining_time: pending_remaining_time };
		}
		
		if (devmode){
		console.log(`DevMode: Cancel Granted for pendingStartTime(elapsed: ${Math.round(pending_elapsed)}ms | required: ${Math.round(pending_required)}ms)`);
		}
		}
		
		if (current_info && current_info.cancelLogic && current_info.rearCancelStartTime && rear_cancel_valid_types){
		const final_tick_rate = (1/2) * (1000 / tick_rate); 	
		const cancel_extra_delay = cancel_delay / clientAction.real_speed;
		const rear_cancel_required = (current_info.rearCancelStartTime / clientAction.real_speed) + final_tick_rate + cancel_extra_delay;
		const rear_cancel_elapsed = Date.now() - lastStartTime;

		if (rear_cancel_elapsed < rear_cancel_required) {
		//we gonna store remaining time to queue up the next attempt for rearcancel types
		const rear_cancel_remaining_time = Math.max(0,rear_cancel_required - rear_cancel_elapsed );

		if (devmode){
		console.log(`DevMode: Cancel Denied for rearCancelTypes(elapsed: ${Math.round(rear_cancel_elapsed)}ms | required: ${Math.round(rear_cancel_required)}ms)`);
		}
		return { allow_cancel: false, cancel_remaining_time: rear_cancel_remaining_time};
		}
		if (devmode){
		console.log(`DevMode: Cancel Granted for rearCancelTypes(elapsed: ${Math.round(rear_cancel_elapsed)}ms | required: ${Math.round(rear_cancel_required)}ms)`);
		}
		}	
		}
		return { allow_cancel: true, cancel_remaining_time: 0 };
	}
	...
	mod.hook('S_ACTION_STAGE', 9, event => {
		if(isMe(event.gameId)) {
			...
			if(info) {
				...
				if(player.attackSpeed > 1 && event.speed === 1 !== !!info.ignoreAttackSpeed && !info.ignoreAttackSpeedWarning &&!info.lockAnimSpeed) {
			...
			// look later to stop it from drop skill when packet arrives earlier.
	...
	function startAction(opts) {
		const info = opts.info,
			...
			if (info.consumeAbnormalnextSkill) {
					const abnormal = info.consumeAbnormalnextSkill.abnormal;
					const categories = info.consumeAbnormalnextSkill.categories;

				if (!categories || categories.length === 0 || categories.includes(clientAction?.category)) {
					if (Array.isArray(abnormal)) {
						for (let id of abnormal) {
							if (abnormality.exists((id))) {
								abnormality.remove((id));
							}
						}
					} else if (abnormal) {
						if (abnormality.exists((abnormal))) {
							abnormality.remove((abnormal));
						}
					}
				}
			}
		
		//Consume Abnormals by skill category. More refined if a skill is being used.	
		...
		// TO DO to keep applying until real abnormal arrives. For now it only applies to the first skill. which is good
		let allow_apply = true;

		if (info.TriggerPreCalculateSpeedAbnormals) {
			const requireGlyphs = info.TriggerPreCalculateSpeedAbnormals.requireGlyphs;

			for (const id in info.TriggerPreCalculateSpeedAbnormals) {
				if (id === 'requireGlyphs') continue;

				const requiredCrests = info.requiredCrests?.[id];   
				const hasCrest = requiredCrests !== undefined
					? Boolean(player.crests?.has(Number(requiredCrests)))
					: true;

				const hasReqGlyphs = requireGlyphs !== undefined
					? Boolean(player.crests?.has(Number(requireGlyphs)))
					: true;

				allow_apply = hasCrest && hasReqGlyphs;

				if (!allow_apply) {
					abnormality.remove?.(id);
					preCalculatedSpeeds.delete(id);
					continue;
				}

				const preAbnormal = info.TriggerPreCalculateSpeedAbnormals[id];

				if (Array.isArray(preAbnormal)) {
					abnormality.add(id, preAbnormal[0], preAbnormal[1]);
				} else {
					abnormality.add(id, preAbnormal, 1);
				}
				
				//check abnormality type
				const AbnormalType = clientAbnormality[id];
				if (AbnormalType && AbnormalType.effects) {
					for (let effect of AbnormalType.effects) {
						if(effect.type === 24) {
							const speedBonus = effect.value !== undefined ? effect.value : 0;
							const speedMethod = effect.method !== undefined ? effect.method : 0; 
							preCalculatedSpeeds.set(id, [1, {bonus: speedBonus, method: speedMethod}]);          
							if(devmode) {
								console.log(`DevMode: PreCalculated speed applied ID: ${id} | Method: ${speedMethod} | Bonus: ${speedBonus}`);
							}
							break;
						}   
					}
		...
		if (info.triggerAbnormalGlyphs){
			const glyphs = info.triggerAbnormalGlyphs;
            const abnormals = glyphs.abnormals;
            const requiredGlyphs = glyphs.requiredGlyphs;


            const has_require_glyphs = requiredGlyphs !== undefined

                ? Boolean(player.crests?.has?.(Number(requiredGlyphs)))
                : true;

            if (devmode){
                console.log(`DevMode: Glyph Abnormal Check | Required: ${glyphs.requiredGlyphs} | Has??: ${has_require_glyphs}`);
         		}

            if (has_require_glyphs){
                for (const id in abnormals){
                     const abnormal_val = abnormals[id];

                    if (Array.isArray(abnormal_val)){
                        abnormality.add(id, abnormal_val[0], abnormal_val[1]);
                    } else{
                        abnormality.add(id, abnormal_val, 1);
                    }
                }
            } else { 
                for (const id in abnormals){
	...
	function sendActionStage(opts) {
		...
		//after speed get declared those will be added and removed when applied.
		//TO DO, apply till real abnormal arrives//refreshes. Currently applies on the first, which does fix Arcane barrage desync.
		if (!info.ignoreAttackSpeed && preCalculatedSpeeds.size > 0) 
			{
       		for (let [abnormalId, entry] of preCalculatedSpeeds.entries())
			{
				switch(entry[1].method)
				{
				case 3:	
				animSpeed *= entry[1].bonus;
				realSpeed *= entry[1].bonus; 
				break;
				case 0:
				break;
				default:
				...
				realSpeed += 0 
					...
					if(devmode)
					{
					console.log(`DevMode: PreCalculated speed applied ID: ${abnormalId}`);
					}
					preCalculatedSpeeds.delete(abnormalId); 
			}		
			}	
			else if(preCalculatedSpeeds.size > 0){//just remove it when ignoreattackspeed = true
			{	
					for (let abnormalId of preCalculatedSpeeds.keys())
					{	
					if(devmode)
					{
					console.log(`DevMode: PreCalculated speed applied but ignored ID: ${abnormalId}`);
					}
					preCalculatedSpeeds.delete(abnormalId);
				}
			}
		...
		//Some skills has weird attackspeed properties so packets align with server.
		if(info.lockAnimSpeed) {
			animSpeed = 1;
		}

		if(mod.settings.debug.skills) debug(`<* S_ACTION_STAGE ${skillId(opts.skill)} Stage: ${opts.stage} ${decimal(animSpeed, 3)}x ${decimal(realSpeed, 3)}x`)
		...
		mod.send('S_ACTION_STAGE', 9, clientAction = {
			...
			stage: opts.stage,
			speed: animSpeed,
			projectileSpeed: (info.lockAnimSpeed || info.ignoreAttackSpeed) ? 1 : realSpeed,
			id: actionNumber,
		...
		clientAction.real_speed = realSpeed;
		...
		// calculates skill length i will need it later.
		...
		//seperate moving charge to fix mana missile full charge not working after chaining from another skill
		if(['defence', 'movingdefence', 'presshit'].includes(info.dcType)) {
			if (!info.noSkipStageCharge){
			opts.stage += 1;
		}
			stageEnd = sendActionStage.bind(null, opts)
			stageEndTime = Date.now() + length
		...
		if (info.dcType === 'movingcharge') {
   			 const maxStages = Array.isArray(info.length) ? info.length.length : 1;

    		if (opts.stage < maxStages && !info.noSkipStage && !info.noSkipStageCharge) {
       		opts.stage += 1;
   			}
	...
	function sendActionEnd(type, distance, dir) {
		...
		//intercept S_Action_END to sychronize and predict packets.

		//TO DO
		// function S_Instant_Move resync
		//TO DO predict abnormal // fixes desync when abnormal arrives later that modifies skill speed.
	...
	// check skill chain if it contains more than 2 digits or above 99 -> do not modify. More refined skill chaining which is needed more mana missile.
