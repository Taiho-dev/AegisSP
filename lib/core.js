...
module.exports = function SPCore(mod) {
	...
	// Current skill
		...
		release_delay = 0,
		// adds delay to connect chains.
		cancel_delay = 0,
		// simulate server tick rate and apply after connect chains, value is in hertz. based on best case scenario half-tick latency.
		tick_rate = 30,
		// Projectiles
	...
	mod.command.add('reloadskills', () => {
		...
		mod.command.add('betapingabc', () => {
		try {	
				offset_end = !(end_modifier - 25);
				mod.command.message(`BETA high ${offset_end ?'on' : 'off'}`);
			}
		catch (error) 
	...
	//code ripped from my old release chain modification, atleast it works!
		...
		if (info.connectChains && clientAction){
			...
			if (chain_info){
				...
				if (prevInfo){
					...
					const required_time = (number / clientAction.speed) / prevInfo.timeRate; 
					const elapsed = Date.now() - lastStartTime;
		...
		// connect chains for abnormals.
		let do_connect_abnormal = false;
		if (info.abnormalConnect && clientAction){
			const current_skill_id = clientAction.skill.id;
			const chain_info = info.abnormalConnect[current_skill_id];
			
			
			if (chain_info){
				const require_abnormal = abnormality.exists(chain_info.abnormal);
				const next_skill_id = chain_info.connectToID;
				const rear_this = chain_info.type === 'rear';
				const has_abnormal = Boolean(require_abnormal);
				const prev_info = skillInfo(clientAction.skill);

				if (prev_info && has_abnormal){
					const number = rear_this ? prev_info.rearCancelStartTime : prev_info.pendingStartTime;
					const required_time = (number / clientAction.speed) / prev_info.timeRate; 
					const elapsed = Date.now() - lastStartTime;
				...
				// Queue Skill ripped from above. //stops spamming and queues the next skill used

		let do_queue = false;
		if (info.queue && clientAction){
			const current_skill_id = clientAction.skill.id;
			const chain_info = info.abnormalConnect[current_skill_id];
...
function canCancelSkill(endReason, nextSkill)
	...
	if (nextSkill && nextSkill.lockCancel && clientAction)
		{
		// stops ninja auto attack spam bug
	const prev_info = skillInfo(clientAction.skill);

		if(prev_info && prev_info.pendingStartTime)
		{
		const timeRate = prev_info.timeRate || 1;
		const pending_time = prev_info.pendingStartTime;
		//no idea if this will make it work but it does!
		const required_time = (pending_time / clientAction.speed) / timeRate;
		const elapsed = Date.now() - lastStartTime;
	...
	function startAction(opts) {
		...
		// if(info.triggerAbnormalDelay){
		// 	const default_delay = Number(info.triggerAbnormal.delay) || 0;
		// 	for(let id in info.triggerAbnormal) {
		// 		if (id === 'delay') continue;

		// 		const abnormal = info.triggerAbnormal[id]
		// 		delay = (Number(this_delay.delay));

		// 		const apply_abnormal = () => {
		// 			if (Array.isArray(abnormal)){
		...
		// 		}};
		// 		if (delay >0){
		// 			mod.setTimeout(apply_abnormal, delay);
		// 		}else{	
		// 			apply_abnormal();
		// 		}
		// 		}
