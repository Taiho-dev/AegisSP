...
(Server bug: Having Noctenium Infusion in inventory causes skill delays)`)
	...
	function startAction(opts) {
		...
		// TO DO to keep applying until real abnormal arrives. For now it only applies to the first skill. which is good enough for now to keep packets synced.
	...
	function sendActionStage(opts) {
		...
		if (info.dcType === 'movingcharge') {
    		...
    		if (opts.stage < maxStages && !info.noSkipStage) {
       		opts.stage += 1;
   			}
