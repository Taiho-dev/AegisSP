...
module.exports = function SPCore(mod) {
	...
	for (let releaseChain of releaseChains)
	{
		...
		if	(	
            ...
            if (prevInfo) {
                const RequiredTime = prevInfo.pendingStartTime / clientAction.speed;
                const Elapsed = Date.now() - lastStartTime;
				
                if (Elapsed >= RequiredTime || event.ReadyToRelease) {                
				...
				//Temporary, will place it somewhere else in the future
					const BestCaseScenario = (1/2) * (1000 / TickRate);	
					const FinalTickRate = BestCaseScenario;

					const AdditionalTime = ReleaseDelay / clientAction.speed;					
                    const RemainingTime = RequiredTime - Elapsed + FinalTickRate + AdditionalTime;
                    
                    mod.clearTimeout(delayedSkillTimeout);
                    delayedSkillTimeout = mod.setTimeout(() => {
					//Fire it if true!
                    event.ReadyToRelease = true;
