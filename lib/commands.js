...
<FONT COLOR="#FFFFFF">debug [skill|loc|abnormal]</FONT> = Toggle debug modes.`)
	...
	function printPing() {
		mod.command.message(`Ping: ${ping.history.length ? `Min=${ping.min.toFixed(4)} Max=${ping.max.toFixed(4)} Avg=${Math.round(ping.avg)} Spread=${(ping.max - ping.min).toFixed(4)} Samples=${ping.history.length}` : '???'}`)
	}
